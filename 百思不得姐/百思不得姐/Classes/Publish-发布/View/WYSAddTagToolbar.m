...
@implementation WYSAddTagToolbar
...
- (void)addButtonClick
{
    WYSAddTagViewController *vc = [[WYSAddTagViewController alloc] init];
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i<self.tagLabels.count; i++) {
        ...
        if (i == 0) { // 最前面的标签
            ...
            // 计算当前行左边的宽度
            CGFloat leftWidth = CGRectGetMaxX(lastTagLabel.frame) + WYSTagMargin;
            // 计算当前行右边的宽度
            ...
            if (rightWidth >= tagLabel.width) { // 按钮显示在当前行
                ...
                tagLabel.x = 0;
                tagLabel.y = CGRectGetMaxY(lastTagLabel.frame) + WYSTagMargin;
            }
        }
    }
    ...
    if (self.topView.width - leftWidth >= self.addButton.width) {
        ...
        self.addButton.x = 0;
        self.addButton.y = CGRectGetMaxY(lastTagLabel.frame) + WYSTagMargin;
    }
...
- (void)createTagLabels:(NSArray *)tags
{
    ...
    for (int i = 0; i<tags.count; i++) {
        ...
        tagLabel.backgroundColor = WYSTagBg;
        tagLabel.textAlignment = NSTextAlignmentCenter;
        tagLabel.text = tags[i];
        tagLabel.font = WYSTagFont;
        // 应该要先设置文字和字体后，再进行计算
        [tagLabel sizeToFit];
        tagLabel.width += 2 * WYSTagMargin;
        tagLabel.height = WYSTagH;
